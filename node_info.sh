...
main() {
	...
	local node_tcp=`cat "${node_dir}anoma-feigenbaum-0.ebb9e9f9013/config.toml" | grep -oPm1 "(?<=^rpc_address = \")([^%]+)(?=\")"`
	local node_tcp="http://${node_tcp}/"
	local status=`wget -qO- "${node_tcp}status" 2>&1`
	local moniker=`jq -r ".result.node_info.moniker" <<< $status`
	...
	local network=`jq -r ".result.node_info.network" <<< $status`
	local node_id=`jq -r ".result.node_info.id" <<< $status`
	...
	local voting_power=`grep -oPm1 "(?<=, voting power: )([^%]+)(?=$)" <<< "$validator_info"`
	if [ -n "$wallet_address" ]; then
...
"$balance" 2>/dev/null
		...
		if [ "$catching_up" = "true" ]; then
			local current_block=`wget -qO- "http://62.171.175.162:26657/abci_info" | jq -r ".result.response.last_block_height"`
